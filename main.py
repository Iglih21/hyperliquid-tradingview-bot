import os
import math
import time
from fastapi import FastAPI, HTTPException
from pydantic import BaseModel
from hyperliquid.exchange import Exchange
from hyperliquid.info import Info
from hyperliquid.utils import constants

# =====================================
# ENV
# =====================================
PRIVATE_KEY = os.getenv("HYPERLIQUID_PRIVATE_KEY")   # API Wallet Private Key
ACCOUNT = os.getenv("HYPERLIQUID_WALLET")           # Your Trust Wallet Address
DEFAULT_LEVERAGE = float(os.getenv("DEFAULT_LEVERAGE", "3"))
MAX_RISK_PCT = float(os.getenv("MAX_RISK_PCT", "0.02"))

if not PRIVATE_KEY or not ACCOUNT:
    raise RuntimeError("Missing HYPERLIQUID_PRIVATE_KEY or HYPERLIQUID_WALLET")

BASE_URL = constants.MAINNET_API_URL

# =====================================
# CLIENTS
# =====================================
info = Info(BASE_URL)
exchange = Exchange(
    private_key=PRIVATE_KEY,
    account_address=ACCOUNT,
    base_url=BASE_URL
)

print("✅ Hyperliquid Exchange connected to", ACCOUNT)

app = FastAPI()

# =====================================
# UTILS
# =====================================
def get_balance():
    state = info.user_state(ACCOUNT)
    return float(state["marginSummary"]["accountValue"])

def get_position():
    state = info.user_state(ACCOUNT)
    for p in state["assetPositions"]:
        pos = p["position"]
        if pos["coin"] == "BTC":
            return float(pos["szi"])
    return 0.0

def get_price():
    return float(info.all_mids()["BTC"])

def get_sz_decimals():
    meta = info.meta()
    for m in meta["universe"]:
        if m["name"] == "BTC":
            return m["szDecimals"]
    return 3

def round_down(v, d):
    f = 10 ** d
    return math.floor(v * f) / f

# =====================================
# API
# =====================================
class Signal(BaseModel):
    action: str  # BUY or SELL
    risk_pct: float = MAX_RISK_PCT
    leverage: float = DEFAULT_LEVERAGE

@app.get("/")
def root():
    return {"ok": True}

@app.post("/webhook")
def trade(signal: Signal):
    side = signal.action.upper()
    if side not in ["BUY", "SELL"]:
        raise HTTPException(400, "Action must be BUY or SELL")

    price = get_price()
    balance = get_balance()
    pos = get_position()
    sz_decimals = get_sz_decimals()

    risk = min(signal.risk_pct, MAX_RISK_PCT)
    notional = balance * risk * signal.leverage
    size = round_down(notional / price, sz_decimals)

    if size <= 0:
        raise HTTPException(400, "Order size too small")

    print("BTC price:", price)
    print("Account balance:", balance)
    print("Calculated size:", size)
    print("Existing position:", pos)

    # Close opposite side
    if pos > 0 and side == "SELL":
        print("Closing long")
        exchange.market_close("BTC")

    if pos < 0 and side == "BUY":
        print("Closing short")
        exchange.market_close("BTC")

    exchange.update_leverage("BTC", signal.leverage)

    is_buy = side == "BUY"

    resp = exchange.market_open("BTC", is_buy, size)

    print("Exchange response:", resp)

    return {
        "status": "sent",
        "side": side,
        "size": size,
        "price": price,
        "exchange_response": resp
    }
